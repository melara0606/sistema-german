                  ...
                  <tr>
                    ...
                    <td>
                            @if($estado == 1 || $estado == "")
                                ...
                                <button class="btn btn-danger btn-xs" type="button" onclick={{ "baja(".$tipocontrato->id.")" }}><span class="glyphicon glyphicon-trash"></span></button>
                                {{ Form::close()}}
                            @else
                                {{ Form::open(['method' => 'POST', 'id' => 'alta', 'class' => 'form-horizontal'])}}
                                <button class="btn btn-success btn-xs" type="button" onclick={{ "alta(".$tipocontrato->id.")" }}><span class="glyphicon glyphicon-trash"></span></button>
                                {{ Form::close()}}
                             @endif
                        </td>
                  </tr>
                ...
                <script>
                    function baja(id)
                    {
                        swal({
                            ...
                            $('#baja').attr('action','http://'+dominio+'/sisverapaz/public/tipocontratos/baja/'+id+'+'+text);
                    ...
                    function alta(id)
                    {
                        swal({
                            ...
                            var dominio = window.location.host;
                            var form = $(this).parents('form');
                            $('#alta').attr('action','http://'+dominio+'/sisverapaz/public/tipocontratos/alta/'+id);
                            //document.getElmentById('baja').submit();
                            $('#alta').submit();
                            swal({
                                type: 'success',
